...
export default function PodcastSinglePage({ params }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try{
                const response = await axios.get(`/wp-json/v1/podcasts/similar/${params.podcast}?per_page=6`);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`/v1/podcast/${params.podcast}`);
    ...
    const playerIsInView = useInView(playerRef , {once : true});
    ...
    return(
        ...
        {/*  podcast section  */}
            <div className="md:w-2/3 xl:w-1/2 mx-auto mt-14">
            {/*  it has 5 rows  */}
            {/*  the image of the podcast  */}
                <motion.div
                    ref={imageRef}
                    initial={{y : 50 , opacity : 0}}
                    animate={imageIsInView ? {y : 0 , opacity : 1} : {}}
                    transition={{duration : 0.5}}
                    className="w-full border-4 border-black p-14">
                    <div className="relative w-full h-[220px] md:h-[330px] lg:h-[460px] xl:h-[420px]">
                        <Image src={podcast?.featured_image} alt="Podcast image" layout="fill" objectFit="cover" className="absolute" />
                    </div>
                </motion.div>
                ...
                <motion.div
                    ref={playerRef}
                    initial={{y : 50 , opacity : 0}}
                    animate={playerIsInView ? {y : 0 , opacity : 1} : {}}
                    transition={{duration : 0.5}}
                    className="w-full rtl mt-5 font-common-heavy text-20px md:text-25px lg:text-30px">
                    {podcast?.title}
                </motion.div>
                ...
                <motion.div
                    ...
                    className="flex flex-row-reverse justify-between items-center mt-5">
                    <div className="flex justify-start items-center rtl w-1/2">
                        <div className="font-common-lg text-16px xl:text-20px">مهمان دار:</div>
                        <div className="font-common-regular text-16px xl:text-20px">{podcast?.meta.host_name}</div>
                    </div>
                    <div className="flex justify-start items-center rtl w-1/2">
                        <div className="font-common-lg text-16px xl:text-20px">مهمان:</div>
                        <div className="font-common-regular text-16px xl:text-20px">{podcast?.meta.guest_name}</div>
                    </div>
                </motion.div>
                {/* player */}
                <div className="mt-14">
                    <audio ref={audioRef} src={podcast?.meta.audio_file} onTimeUpdate={handleTimeUpdate}/>
                    {/* Progress Bar */}
                    <motion.div
                        ref={playerRef}
                        initial={{y : 50 , opacity : 0}}
                        ...
                        <div className="absolute w-5 h-5 rotate-45 bg-black -top-[6px]" style={{left : `${progress}%`}}></div>
                        {/* the time section */}
                    ...
                    <motion.div
                        ...
                        <div onClick={skipBackward} className="cursor-pointer">
                            {/* back button */}
                        ...
                        >
                            {isPlaying && currentTime < duration ?
                                ...
                                <svg width="39" height="46" viewBox="0 0 39 46" fill="none"
                                     xmlns="http://www.w3.org/2000/svg" className="ml-2" onClick={play}>
                                    <path d="M34.7892 22.9999L3.16176 41.9764V4.02346L34.7892 22.9999Z" fill="#FFFCF7"
                                          stroke="#FFFCF7" stroke-width="6.08824"
                                    />
                                </svg>
                            }
                        </button>
                        <div onClick={skipForward} className="cursor-pointer">
                            {/*  forward button  */}
                            <svg width="40" height="45" viewBox="0 0 40 45" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                                <path
        ...
        {/*  podcast text  */}
            <div className="w-full mt-14">
                ...
                <motion.div
                    ...
                    className="border-t-4 border-b-4 border-black whitespace-pre-wrap rtl font-common-lg
                ...
                <div className="mt-14 mb-14">
                    ...
                    <motion.div
                        ...
                        <ArrowLink title="همه نشست ها" path="/podcasts"/>
                    </motion.div>
                    <div className="flex flex-col md:flex-row mt-7 md:flex-wrap">
                        {
                            data?.map((data , index) => (
